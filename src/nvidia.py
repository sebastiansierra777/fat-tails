...
# Load the data
df = pd.read_excel(excel_path, sheet_name="nvidia")
...
with pd.ExcelWriter(
    ...
    df.to_excel(writer, sheet_name="nvidia_log", index=False)

print("\nData Frame successfully exported to working sheet")
...
rows = [
    ...
    ("standard dev", describe_stats["std"]),
    ("minimum", describe_stats["min"]),
    ("25%", describe_stats["25%"]),
    ("50%", describe_stats["50%"]),
    ("75%", describe_stats["75%"]),
    ("maximum", describe_stats["max"]),
    ("skewness", skewness),
